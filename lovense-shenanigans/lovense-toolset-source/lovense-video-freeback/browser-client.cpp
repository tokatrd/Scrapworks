...
//Temporarily support one
static lovense::proc::cef_special_effects_callback g_special_effects_callback{ nullptr };
static std::mutex                                  g_special_effects_mutex;

void add_special_effects(lovense::proc::cef_special_effects_callback callback)
{
	std::lock_guard<std::mutex> lk{ g_special_effects_mutex };
	g_special_effects_callback = callback;
}

void remove_special_effects(lovense::proc::cef_special_effects_callback callback)
{
	std::lock_guard<std::mutex> lk{ g_special_effects_mutex };
	if (callback == nullptr || callback == g_special_effects_callback) {
		g_special_effects_callback = nullptr;
	}
}

static void send_special_effects(const std::string& data)
{
	std::lock_guard<std::mutex> lk{ g_special_effects_mutex };
	if (g_special_effects_callback) {
		g_special_effects_callback(data);
	}
}


static void get_head_object_points(json11::Json& value)
{
	calldata_t data;
	calldata_init(&data);
	calldata_set_ptr(&data, "result", (void*)(&value));
	auto handle = obs_get_proc_handler();
	if (handle) {
		proc_handler_call(handle, "__lovense_smart_object_GetHeadPoints", &data);
	}
	calldata_free(&data);
}

static void forward_message(std::string eventName, std::string jsonString, CefRefPtr<CefBrowser> browser)
{
...
static void handle_request(CefRefPtr<CefBrowser> browser, const std::string& message)
{
	using namespace lovense;

	json11::Json value;
	std::string err_message;
	json11::Json request = json11::Json::parse(message, err_message);
	if (request.is_null()) {
		value = json11::Json::object{
		   {"code",    error::lvs_ParseJson},
		   {"message", err_message.empty() ? "json parse error" : err_message.c_str()}
		};
	}

	if (value.is_null() &&  ((!request["module"].is_string()) || request["module"].string_value().empty())) {
		value = json11::Json::object{
		   {"code",    error::lvs_ParamsBad},
		   {"message", "params error"}
		};
	}

	if (value.is_null() && ((!request["func"].is_string()) || request["func"].string_value().empty())) {
		value = json11::Json::object{
		   {"code",    error::lvs_ParamsBad},
		   {"message", "params error"}
		};
	...
	if (value.is_null()) {
		...
		json11::Json resp_data;
		calldata_set_int(&data, "identifier", id);
		calldata_set_ptr(&data, "request", (void*)(&request));
		calldata_set_ptr(&data, "response", (void*)(&resp_data));
		auto handle = obs_get_proc_handler();
		bool is_handle{ false };
		int  code = error::lvs_MissModule;
		std::string error_message = "not find module handler";
		if (handle) {
			try {
				std::string full_name = "__cef_" + request["module"].string_value() + "_" + request["func"].string_value();
				is_handle = proc_handler_call(handle, full_name.c_str(), &data);
			}
			catch (const std::exception& e) {
				code = error::lvs_Exception;
		...
		if (is_handle)
		{
			if (!resp_data.is_null()) {
				value = json11::Json::object{
				   ...
				   {"data",    resp_data["data"].is_null() ? json11::Json::object{} : resp_data["data"]},
				   {"code",    resp_data["code"].is_null() ? 0 : resp_data["code"]},
				   {"message", resp_data["message"].is_null() ? "ok" : resp_data["message"]},
				};
			}
		}
		else {
	...
	if (!value.is_null()) {
		forward_message("event__" + request["module"].string_value(), value.dump(), browser);
	}
}
	...
	CefRefPtr<CefProcessMessage> message)
{
	...
	case ControlLevel::None:
		...
		else if (name == "sendSpecialEffects") {
			std::string frame = input_args->GetString(1).ToString();
			send_special_effects(frame);
		}
		else if (name == "getHeadObjectPoints") {
			get_head_object_points(json);			
		}
		else if (name == "lovense_request") {
			std::string message = input_args->GetString(1).ToString();
			handle_request(browser, message);
		}
        else if (name == "setCurrentScene"){
            std::string message = input_args->GetString(1).ToString();
            handle_request(browser, message);
	...
	execute_args->SetString(result_index, json.dump());
			    ...
			    int height)
{
	...
	if (sharing_available) {
		bs->realFlip = false;//wystan add for hardware acceleration open but not support  230406
		//return;
	}
#endif
				       ...
				       void *shared_handle)
{
	...
	if (bs->libobs_gs_texture_open_nt_shared) {
		typedef gs_texture_t* (*gs_texture_open_type)(uint32_t);
		gs_texture_open_type func = (gs_texture_open_type)(bs->libobs_gs_texture_open_nt_shared);
		bs->texture = func((uint32_t)(uintptr_t)shared_handle);
	}
	else {
				     ...
				     const CefString &source, int line)
{
	...
	blog(errorLevel, "[lovense-video-freedback] %s: %s (%s:%d)", code,
