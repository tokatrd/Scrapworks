...
#if defined(ENABLE_BROWSER_PANELS)||defined(__APPLE__)
...
#if defined(DEF_TOOLSETS) && defined(_WIN32) && (LIBOBS_API_VER < MAKE_SEMANTIC_VERSION(28, 0, 0))
	libobs = LoadLibraryA("obs.dll");
	if (libobs) {
		libobs_gs_texture_open_nt_shared = GetProcAddress(libobs, "gs_texture_open_nt_shared");
	}
#endif
}
...
#if defined(DEF_TOOLSETS) && defined(_WIN32) && (LIBOBS_API_VER < MAKE_SEMANTIC_VERSION(28, 0, 0))
	if (libobs) {
		if (FreeLibrary(libobs) == 0)
		{
			int errorCode = GetLastError(); 
			blog(LOG_WARNING, "FreeLibrary libobs failed. error code: %d", errorCode);			
		}

		libobs = nullptr;
		libobs_gs_texture_open_nt_shared = nullptr;
	}
#endif
}
...
void BrowserSource::Destroy()
{
	...
	*p_prev_next = next;
	...
	{
		std::lock_guard<std::mutex> lk{ source_lock };
		if (this->source) {
...
bool BrowserSource::CreateBrowser()
{
	return QueueCEFTask([this]() {
		...
		bool hwaccel = false;
#endif
		realFlip = hwaccel;//wystan add for hardware acceleration open but not support  230406
...
#endif
		auto browser = CefBrowserHost::CreateBrowserSync(//wystan mark for create cef browser 230324
		...
		{
			std::lock_guard<std::mutex> lk{ source_lock };
			if (source && obs_source_showing(source))
				is_showing = true;
		}		
...
void BrowserSource::Update(obs_data_t *settings, bool fixed_url)
{
	if (settings) {
		bool n_is_local;
		int n_width;
...
#else
		if (fixed_url) {
			//wystan modify for just get default url when url error 230324
			n_url = obs_data_get_default_string(settings, "url"); 
		}
		else {
...
void BrowserSource::Render()
{
	bool flip = false;
#ifdef ENABLE_BROWSER_SHARED_TEXTURE
	flip = realFlip;//hwaccel;//wystan add for hardware acceleration open but not support  230406
#endif
...
static void ExecuteOnBrowsers(BrowserFunc func, int bs_id)
{
	lock_guard<mutex> lock(browser_list_mutex);
	BrowserSource* bs = first_browser;
	while (bs) {
		BrowserSource* bsw = reinterpret_cast<BrowserSource*>(bs);
		if (bsw && ((bs_id < 0) || (bs_id == bsw->GetBrowser()->GetIdentifier())))
		{
			bsw->ExecuteOnBrowser(func, true);
		}		
		bs = bs->next;
...
void DispatchJSEvent(std::string eventName, std::string jsonString, int id)
{
	const auto jsEvent = [=](CefRefPtr<CefBrowser> cefBrowser) {
		CefRefPtr<CefProcessMessage> msg =
			CefProcessMessage::Create("DispatchJSEvent");
