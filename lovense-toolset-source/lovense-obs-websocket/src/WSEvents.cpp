...
std::atomic_bool g_isOBSExiting = false;
...
void WSEvents::FrontendEventHandler(enum obs_frontend_event event, void* private_data) {
	...
	switch (event) {
		...
		case OBS_FRONTEND_EVENT_EXIT:
			...
			break;
#ifndef DEF_TOOLSETS 
		// TODO: Add OBS Windows display state monitoring
		case OBS_FRONTEND_EVENT_WINDOWS_SHOW_CHANGED:
			owner->OnWindowsShowChanged();
			break;
#endif
	}
}
...
{
	//cita:modify don't use the Heartbeat
	//return;
	if (!_srv->isListening()) {
		return;
	}
...
void WSEvents::OnSceneChange() {
	...
	obs_data_set_string(data, "scene_name", obs_source_get_name(currentScene));
...
void WSEvents::OnWindowsShowChanged()
{
#ifndef DISABLE_QT
	...
	broadcastUpdate("OBSApplicationShowChanged", data);
#endif
}
...
#if 1
    //TODO:cita modify don't send this message
    return;
#endif
...
void WSEvents::OnSourceAudioMixersChanged(void* param, calldata_t* data) {
	...
	for (long long i = 0; i < MAX_AUDIO_MIXES; i++) {
		...
		obs_data_set_bool(item, "enabled", (((long long)1 << i) & audioMixers));
...
#else
	QString hexValue = string_util::format("0x%X", audioMixers);
#endif
...
void WSEvents::OnSceneItemDelete(void* param, calldata_t* data) {
	// TODO: Add when source is deleted from scene
	if (g_isOBSExiting)
		return;
	auto instance = reinterpret_cast<WSEvents*>(param);
	if(instance)
...
#else
	obs_data_set_string(broadcastData, "realm", realm.toUtf8());
#endif // !DISABLE_QT	
